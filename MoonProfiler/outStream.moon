...
with outStream
  ...
  .writeJsonArr = (table) ->
    ...
    for k, v in pairs table
      ...
      t = type v
      if t == "table"
        insert str, outStream.writeJsonArr v
      elseif t == "number"
  ...
  .writeMark = (thredId, mark) ->
    ...
    if mark.args
      stream\write [[,"args":]]
      stream\write outStream.writeJsonArr(mark.args)
  ...
  .writeProfile = (thredId, profile) ->
    ...
    if profile.args
      stream\write [[,"args":]]
      stream\write outStream.writeJsonArr(profile.args)
  ...
  .writeCounter = (thredId, counter) ->
    ...
    if counter.args
      stream\write [[,"args":]]
      stream\write outStream.writeJsonArr(counter.args)
  ...
  .writeMetadata = (thredId, metadata) ->
    ...
    for key, name in pairs(metadata)
      if pb then outStream.pushBack!
      outStream.jsonMetadata thredId, key, name
      if flush then stream\flush!
      pb = true
